...
pren_net(x::Img) = Float32.(expanddims(x.img))

"Distance between two images is feature representation"
function Omega.dist(x::Img, y::Img)::Real
  xfeatures = SqueezeNet_.squeezenet(pren_net(x))
  yfeatures = SqueezeNet_.squeezenet(pren_net(y))
